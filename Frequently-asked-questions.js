...
function flattenObjectStructure(object) {
  const result = {};

  for (const key in object) {
    // Check if the key belongs to the object (not prototype)
    if (object.hasOwnProperty(key)) {
      if (
        ...
        const temp = flattenObjectStructure(object[key]);
        // Merge flattened nested keys with dot notation
        for (const nestedKey in temp) {
          result[key + "." + nestedKey] = temp[nestedKey];
        }
      } else {
        result[key] = object[key]; // Add non-object values directly
      }
    }
  }
...
const dictionary = ["CODGE", "ODG", "LODGES", "SODG", "dodge", "mODJ", "LODGESSSS"];
const substringToSearch = "ODG";

// Use Array's filter method to find matching strings
const filteredResults = dictionary.filter((word) => word.includes(substringToSearch));
...
const [productWithMostVowels, vowelCount] = productList.reduce((acc, currentProduct) => {
